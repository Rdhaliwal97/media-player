function addSongToDisplay(metadata, songObj) {
	...
	let title = metadata.common.title ? metadata.common.title : path.posix.basename(path.basename(audioFile), extName);
	let artist = metadata.common.artist ? metadata.common.artist : 'Unknown artist';
...
document.querySelector('#songList').addEventListener('click', (event) => {
	let target = event.target.tagName.toLowerCase() == 'div' ? event.target : event.target.parentNode;
